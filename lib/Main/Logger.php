...
class Logger
{
    ...
    private static function GetLogFolder()
    {
    ...
    public static function createLogFile()
    {
        ...
        static::$log_file = self::GetLogFolder()."/log-{$time}.log";

        //Check if directory /logs exists
        if (!file_exists(self::GetLogFolder())) {
            mkdir(self::GetLogFolder(), 0777, true);
        }
    ...
    public static  function writeLog($args = [])
    {
        ...
        fwrite(static::$file, "{$timeLog}{$pathLog}{$lineLog}: {$severityLog} - {$messageLog} {$contextLog} {$delimiter}" . PHP_EOL);
